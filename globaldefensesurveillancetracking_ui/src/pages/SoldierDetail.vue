					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateSoldier()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div id="app">
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Soldiers = true">Add</base-button>
								              <modal :show.sync="modal2Soldiers">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Soldier Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SoldierId" type="text" placeholder="Enter SoldierId" v-model="soldierDetails.soldierId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Name" type="text" placeholder="Enter Name" v-model="soldierDetails.name"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Rank</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Rank" type="text" placeholder="Enter Rank" v-model="soldierDetails.rank"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateSoldier() {
      ...
      const jsonData = JSON.stringify(this.soldierDetails);
      const res = await SoldierService.update(jsonData);
...
//        this.getAllSoldiers();
    ...
    async getSoldierDetails() {
      try {
			let response = await SoldierService.get(this.soldierId);
			this.soldierDetails = response.data;
